...
double AverageTerrain(site_t * source, double azimuthx, double start_distance, double end_distance)
{
	...
	if (dem_get_elevation_loc(destination)<-4999.0)
	{
	...
	if (fd1!=NULL)
	{
		...
		do
		{
			...
			if (z==0)  /* No duplicate found */
				dem_add_elevation_pos(xpix*dpp, ypix*dpp, height);
...
void WritePPM(char *filename, unsigned char geo, unsigned char kml, unsigned char ngs, site_t **xmtr, unsigned char txsites)
{
	...
	if (kml && geo==0)
	{
		...
		for (x=0; x<txsites; x++)
		{
			...
			fprintf(fd,"        <coordinates>%f,%f,%f</coordinates>\n",(site_get_lon_deg(xmtr[x]) < 180.0 ? -site_get_lon_deg(xmtr[x]) : 360.0-site_get_lon_deg(xmtr[x])), site_get_lat_deg(xmtr[x]), FOOT_PER_METERS*site_get_alt(xmtr[x]));
			fprintf(fd,"      </Point>\n");
			fprintf(fd,"     </Placemark>\n");
		}
...
void GraphHeight(site_t * source, site_t * destination, char *name, unsigned char fresnel_plot, unsigned char normalized)
{
	...
	for (x=0; x<path.length-1; x++)
	{
		...
		if ((height+clutter*FOOT_PER_METERS)>maxheight)
			maxheight=height+clutter*FOOT_PER_METERS;
...
void PathReport(site_t * source, site_t * destination, char *name, char graph_it)
{
	...
	else
	{
		...
		fprintf(fd2,"Antenna height: %.2f feet AGL / %.2f feet AMSL\n",site_get_alt(source)*FOOT_PER_METERS, site_get_alt(source)*FOOT_PER_METERS+FOOT_PER_METERS*dem_get_elevation_loc(source));
	}
	...
	else
	{
		...
		fprintf(fd2,"Antenna height: %.2f feet AGL / %.2f feet AMSL\n",site_get_alt(destination)*FOOT_PER_METERS, site_get_alt(destination)*FOOT_PER_METERS+FOOT_PER_METERS*dem_get_elevation_loc(destination));
	}
